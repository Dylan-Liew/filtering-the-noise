...
class QualityScoringLayer(BaseLayer):
    ...
    def _analyze_informativeness_semantic(self, text: str, threshold: float = 0.3) -> Dict[str, Any]:
        ...
        try:
            ...
            # Informativeness score as the proportion of templates covered
            base_informativeness_score = len(matched_templates) / len(self.informative_templates)
            
            # Boost score based on high-confidence matches (more generous)
            confidence_boost = min(high_confidence_matches * 0.15, 0.4)
            ...
            descriptive_score = sum(1 for word in descriptive_indicators if word in text.lower()) / len(descriptive_indicators)
